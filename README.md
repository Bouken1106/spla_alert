...
ブキ推定を有効にした場合は、stat.ink の Splatoon 3 weapon API からブキ名、API info ページから画像URLを取得します。各画像はテンプレートサイズへ正規化し、透明部分とグレースケールからエッジ特徴を作ります。実フレーム側は各イカ/タコアイコン中央の低彩度・暗部・明部をブキ候補として抽出し、同じサイズのエッジ特徴に変換します。照合はテンプレートと実フレームのエッジ間の双方向Chamfer距離でスコア化し、信頼度が `weapons.confidence_threshold` 以上の場合だけ `weapon` として返します。
