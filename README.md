...
- `--weapons` 指定時は、外部から取得したブキ画像テンプレートを使って各プレイヤーのブキを推定
- AverMedia/ReCentral の映像を Ubuntu 側に持ってきた入力を読み取り
- スペシャル発光の識別は未実装。ただし発光していても「生存」として扱う想定
...
Docker の権限エラーが出る場合は、各 `docker` コマンドの先頭に `sudo` を付けてください。
...
ReCentral から以下の RTMP URL に送ります。
...
MediaMTX ではなく nginx-rtmp を使う場合の設定例:
...
```bash
spla-alert run --source /dev/video0 --weapons
```

初回は stat.ink から Splatoon 3 のブキ一覧と画像テンプレートを取得し、`local_assets/weapons/stat_ink_weapon3` に保存します。次回以降はこのローカル画像を使います。テンプレートを取り直したい場合:

```bash
spla-alert run --source /dev/video0 --weapons --refresh-weapons
```
...
- `weapons.enabled`: config だけでブキ推定を有効にするか。CLI では `--weapons` でも有効化できる
- `weapons.cache_dir`: 外部から取得したブキ画像テンプレートの保存先
- `weapons.confidence_threshold`: ブキ推定を採用する信頼度の下限。上げると誤推定を出しにくくなる一方、`weapon: null` が増える
- `weapons.crop_*_ratio`: 各アイコン枠からブキ照合に使う中央領域。配信レイアウトや解像度でブキだけを拾えていない場合に調整する
...
試合ごとに味方色・敵色が変わっても、色そのものを固定していないため動きます。味方と敵の色が必ず異なることは、スペシャル判定やブキ推定を強化するときの追加情報として使えます。
...
ブキ推定を有効にした場合は、stat.ink の Splatoon 3 weapon API からブキ名、API info ページから画像URLを取得します。各画像はテンプレートサイズへ正規化し、透明部分とグレースケールからエッジ特徴を作ります。テンプレートは倍率違いと左右反転も持たせ、HUD上の縮尺差や向き違いに少し耐えるようにしています。実フレーム側は各イカ/タコアイコン中央の低彩度・暗部・明部をブキ候補として抽出し、同じサイズのエッジ特徴に変換します。照合はテンプレートと実フレームのエッジ間の双方向Chamfer距離でスコア化します。上位候補が僅差の場合は誤推定を出しやすいため、信頼度が `weapons.confidence_threshold` 以上の場合だけ `weapon` として返します。
...
ローカル検証用に、HUD が検出できた対戦画面画像とスロット切り出しは `local_assets/battle_screens/web_fixtures` に保存できます。この作業ツリーでは 13 枚の対戦画面と 104 個のスロット切り出しを保存済みです。`local_assets/` は外部由来の画像データ置き場として `.gitignore` 対象です。
...
- ブキ推定精度向上: ステージやチーム色の影響を抑える前処理、同系統ブキの再ランキング、軽量分類器の追加
